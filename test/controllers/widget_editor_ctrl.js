describe('Controller WidgetEditorCtrl:', function() {
    ...
    it('add: should not add widget with empty name', function() {
    ...
    it('add: should add widget', function() {
        expect($scope.widgets.list.length).toEqual(2);
        $scope.add('new widget');
        expect($scope.widgets.list.length).toEqual(3);
    });

    it('add: should add widget and put it on edit mode', function() {
        expect($scope.editing).toEqual(null);
        $scope.add('second widget');
        expect($scope.editing.name).toEqual('second widget');
    });

    it('edit: should not set widget on edit mode if is not a custom widget', function() {
        expect($scope.editing).toEqual(null);
        $scope.edit(data.widgets[0].name)
        expect($scope.editing).toEqual(null);
    });

    it('edit: should not set widget on edit mode if widget is already being edited', function() {
        expect($scope.editing).toEqual(null);
    ...
    it('addAttr: should not add attr to the widget if widget is not on edit mode', function() {
    ...
    it('addAttr: should add attr to the widget if widget is on edit mode', function() {
    ...
    it('removeAttr: should not remove attr to the widget if widget is not on edit mode', function() {
        $scope.newAttr = 'new attr';
        expect($scope.editing).toEqual(null);
        $scope.removeAttr();
        expect($scope.editing).toEqual(null);
    ...
    it('removeAttr: should remove attr to the widget if widget is on edit mode', function() {
        $scope.editing = $scope.widgets.list[0];
        expect($scope.editing.attrs.length).toEqual(3);
        $scope.removeAttr();
